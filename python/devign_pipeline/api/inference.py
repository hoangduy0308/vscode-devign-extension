...
class ModelWrapper:
    def __init__(
        ...
        ensemble_config_path: Path = None,
        calibrator_path: Path = None,
        threshold: float = None,
        use_graph_slicing: bool = True,
        use_graph_postprocessing: bool = True,
    ) -> None:
        # If model_dir is provided, resolve all paths relative to it
        if model_dir is not None:
            model_dir = Path(model_dir)
            self.model_paths = model_paths or list(model_dir.glob("*.pt")) or [model_dir / "best_v2_seed42.pt"]
            self.vocab_path = vocab_path or (model_dir / "vocab.json")
            self.config_path = config_path or (model_dir / "config.json")
            self.feature_stats_path = feature_stats_path or (model_dir / "feature_stats.json")
            self.ensemble_config_path = ensemble_config_path or (model_dir / "ensemble_config.json")
            if not self.ensemble_config_path.exists():
                self.ensemble_config_path = model_dir.parent / "ensemble_config.json"
            self.calibrator_path = calibrator_path or (model_dir / "calibrator.joblib")
        else:
            self.model_paths = model_paths or MODEL_PATHS
            self.vocab_path = vocab_path or VOCAB_PATH
            self.config_path = config_path or CONFIG_PATH
            self.feature_stats_path = feature_stats_path or FEATURE_STATS_PATH
            self.ensemble_config_path = ensemble_config_path or ENSEMBLE_CONFIG_PATH
            self.calibrator_path = calibrator_path or CALIBRATOR_PATH
        ...
        # Use custom threshold if provided, otherwise use from ensemble config
        if self._custom_threshold is not None:
            self.threshold = float(self._custom_threshold)
        else:
    ...
    def _aggregate_to_lines(
        ...
        for line, score in sorted(line_scores.items(), key=lambda x: -x[1]):
            ...
            if self._is_comment_line(snippet):
                continue
    ...
    def _is_comment_line(self, line: str) -> bool:
        ...
        if stripped.startswith('/*') or stripped.startswith('*') or stripped.endswith('*/'):
            return True
        # Full block comment on single line
