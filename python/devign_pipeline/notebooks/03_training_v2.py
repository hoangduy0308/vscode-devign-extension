...
# ===== CONFIGURATION =====
NUM_SEEDS = 1  # Single seed for faster training
...
print(f"Config: vocab={data_config['vocab_size']}, samples={data_config.get('train_samples', 'N/A')}")
...
# Detect actual vuln_dim from data
actual_vuln_dim = train_ds_base.vuln_features.shape[1] if train_ds_base.vuln_features is not None else 26
actual_slice_feat_dim = 0
if train_ds_base.slice_vuln_features is not None and train_ds_base.slice_rel_features is not None:
    actual_slice_feat_dim = train_ds_base.slice_vuln_features.shape[-1] + train_ds_base.slice_rel_features.shape[-1]
print(f"Detected vuln_dim={actual_vuln_dim}, slice_feat_dim={actual_slice_feat_dim}")
...
def train_single_model(seed, train_ds, val_ds, n_neg, n_pos, data_config):
    ...
    model = HierarchicalBiGRU(
        ...
        vuln_dim=actual_vuln_dim,
        slice_feat_dim=actual_slice_feat_dim if actual_slice_feat_dim > 0 else 52,
        gate_init=GATE_INIT
    ).to(DEVICE)
